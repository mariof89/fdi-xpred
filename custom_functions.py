...
class HomeAwayOffensiveShotsGoals:
    ...
    def instantiate_figure(self):
        ...
        pitch = VerticalPitch(pitch_type='custom', pitch_length=105, pitch_width=68, line_zorder=12, half=True)
...
class HomeAwayDefensiveEvents:
    ...
    def instantiate_figure_heatmap(self):
        figure, ax = plt.subplots(nrows=1, ncols=2, figsize=(15, 15))
        pitch = VerticalPitch(pitch_type='custom', pitch_length=105, pitch_width=68, line_zorder=12)

        for idx, event in enumerate(stats.defensive_events):
            df_current_event_home = self.df_home[self.df_home['Descrizione'] == f'{event}']
            df_current_event_away = self.df_away[self.df_away['Descrizione'] == f'{event}']
            # terzo grafico: Home Shots Heatmap
            pitch.draw(ax=ax[0])
        ...
        ax[0].set_title(f'{self.team_home} Heatmap of {_considered_defensive_events} events')
        ...
        ax[1].set_title(f'{self.team_away} Heatmap of {_considered_defensive_events} events')
