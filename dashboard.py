...
from custom_functions import HomeAwayOffensiveShotsGoals, HomeAwayDefensiveEvents
...
def truncate_string(full_name: str):
    if len(full_name.split(' ')) == 1:
        return full_name
    _ret = ' '.join([f'{l[0]}' if i == 0 else f'{l}' for i, l in enumerate(full_name.split(' '))])
    return _ret
...
with st.spinner("Wait a moment..."):
    ...
    df_simulated_matches = read_csv(path_simulated_matches).set_index('game_id')

    df_players = read_csv(path_players)

    # ** !! Fix per Samuele Ricci: da togliere nelle righe dell'empoli
    df_players.drop(df_players[(df_players.opta_id == 450023) & (df_players.game_id == 2229098)].index, inplace=True)
    ...
    game_id_match_all = group_by_sum(df_players,
                                     ['game_id', 'team_home', 'team_away', 'matchday'],
                                     use_for_index=False).drop_duplicates('game_id').sort_values(
        ['matchday', 'team_home'])

    # DF con id di tutti i team
    # team_id, team
    team_id_name_logo = df_players.set_index('team_id')['team'].reset_index().sort_values('team_id').drop_duplicates()
...
with match_selection_expander:
    ...
    matches = [f"Select Match of round {selected_round}"] + match_list_by_selected_round['match'].tolist()
...
tab_attacking_metrics, metrics, prediction = st.tabs(
    ["Attacking Metrics", "Metrics", "Predictions"])
...
df_prev_h_played_ha = filter_all_matches_by_team(df=df_previous_tot, selected_round=selected_round, team=team_home)
...
ren_columns = create_dict_for_renaming(df_prev_h_played_ha, endings={'_x': '_home', '_y': '_away'})

print(ren_columns)

df_prev_h_played_ha.rename(columns=ren_columns, inplace=True)

df_prev_a_played_ha = filter_all_matches_by_team(df=df_previous_tot, selected_round=selected_round, team=team_away)
...
with tab_attacking_metrics:
    ...
    df_ipo_by_match_home_single_row = pd.merge(
        ...
        right=df_ipo_by_match_home[df_ipo_by_match_home['home_away'] == 'A'][['weight', 'lk']],
        left_on='lk',
        right_on='lk',
        how='inner'
    )
    ...
    df_ipo_by_match_home_only_team_home['against_team_logo'] = df_ipo_by_match_home_only_team_home.apply(
        lambda x: team_id_name_logo.set_index('team').loc[f"{x['team_home']}"].team_logo
    ...
    # df_ipo_by_match_home_ha_data = pd.merge(
    #    left=df_ipo_by_match_home_only_team_home[['matchday', 'team_home', 'team_away', 'team', 'weight', 'home_away']],
    #    right=df_ipo_by_match_home_only_other_team[['weight']],
    #    left_on='lk',
    #    right_on='lk',
    #    how='inner'
    # )
    # st.write(df_ipo_by_match_home)
    # st.write(df_ipo_by_match_home_only_team_home)
    # st.write(df_ipo_by_match_home_only_other_team)
    # st.write(df_ipo_by_match_home_ha_data)
    # df_ipo_by_match_home_ha_data.rename(
    ...
    df_ipo_by_match_away_single_row = pd.merge(
        left=df_ipo_by_match_away[df_ipo_by_match_away['home_away'] == 'H'][
            ['matchday', 'team_home', 'team_away', 'weight', 'lk']],
        right=df_ipo_by_match_away[df_ipo_by_match_away['home_away'] == 'A'][['weight', 'lk']],
        left_on='lk',
        right_on='lk',
        how='inner'
    )
    ...
    df_ipo_by_match_away_ha_data = pd.merge(
        left=df_ipo_by_match_away_only_team_away[['matchday', 'team_home', 'team_away', 'weight', 'home_away']],
    ...
    print(df_spline_home)
    # df_spline_home['color'] = (df_spline_home.y_considered_team > df_spline_home.y_other_team).apply(
    #   lambda x: 'steelblue' if x else 'orange')
    ...
    dots_ipo = alt.Chart(df_ipo_by_match_home_single_row).mark_circle(size=60,
        ...
        color=alt.Color('home_away:N', title=None, sort='-x', scale=color_scale_home_away),
        tooltip=['team_home', 'team_away', 'ipo_home:Q', 'ipo_away:Q']
    )

    ipochart_home = ipochart_area_away + ipochart_line_home + ipochart_area_home + ipochart_line_match + dots_ipo + team_logos  # ipochart_area_away + ipochart_line_home + ipochart_area_home + dots_ipo + team_logos  # (team_logos + dots_ipo)

    ipo_bars = alt.Chart(df_prev_h_played_ha).mark_bar(size=8,
    ...
    with home_col:
        _all = alt.vconcat(ipochart_home, ipo_bars).configure_view(
    ...
    with away_col:
        ...
        ipo_bars = alt.Chart(df_prev_a_played_ha).mark_bar(size=8,
                                                           cornerRadiusTopLeft=3,
                                                           cornerRadiusTopRight=3,
                                                           cornerRadiusBottomRight=0,
                                                           cornerRadiusBottomLeft=0).encode(
        ...
        _all = alt.vconcat(
            ipochart_area_home + ipochart_area_away + ipochart_line_away + ipochart_line_match + team_logos + dots_ipo,
            ipo_bars
        ).configure_view(
            stroke=None).configure_axis(grid=False).properties(
            padding=0
...
team_id_home = team_id_name_logo[team_id_name_logo.team == team_home]['team_id'].values[0]
team_id_away = team_id_name_logo[team_id_name_logo.team == team_away]['team_id'].values[0]
...
metrics_to_be_considered = attacking_metrics_p90 + defensive_metrics_p90 + passing_metrics_p90 + physical_metrics_p90

for _m in metrics_to_be_considered:
    _df_home = df_players_home_team_minutes_filtered.sort_values(_m, ascending=False).head(5)
    _df_away = df_players_away_team_minutes_filtered.sort_values(_m, ascending=False).head(5)
...
df_shots_home = df_previous_selected_round_home[df_previous_selected_round_home['Descrizione'] == 'Tiro Fatto']
df_goals_home = df_previous_selected_round_home[df_previous_selected_round_home['Descrizione'] == 'Rete Fatta']

df_shots_away = df_previous_selected_round_away[df_previous_selected_round_away['Descrizione'] == 'Tiro Fatto']
df_goals_away = df_previous_selected_round_away[df_previous_selected_round_away['Descrizione'] == 'Rete Fatta']
...
df_goals_against_home = df_previous_selected_round_home[df_previous_selected_round_home['Descrizione'] == 'Rete Subita']
df_shots_against_home = df_previous_selected_round_home[df_previous_selected_round_home['Descrizione'] == 'Tiro Subito']

df_goals_against_away = df_previous_selected_round_away[df_previous_selected_round_away['Descrizione'] == 'Rete Subita']
df_shots_against_away = df_previous_selected_round_away[df_previous_selected_round_away['Descrizione'] == 'Tiro Subito']
